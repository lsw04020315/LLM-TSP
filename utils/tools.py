...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            batch_y = batch_y[:, -args.pred_len:, f_dim:].float().to(device)
...
def test(model, test_data,setting, test_loader, args, device, itr):
    preds = []
    trues = []
    inputx = []
    # mases = []
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            if args.use_amp:
                ...
                if args.output_attention:
                    ...
                    outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
            f_dim = -1 if args.features == 'MS' else 0
            outputs = outputs[:, -args.pred_len:, f_dim:].float().detach().cpu().numpy()
            batch_y = batch_y[:, -args.pred_len:, f_dim:].float().detach().cpu().numpy()
            pred = outputs
            true = batch_y
            preds.append(pred)
            trues.append(true)
            inputx.append(batch_x.detach().cpu().numpy())
            # input_embedding = outputs1.detach().cpu().numpy()
            # prompted_embedding = outputs2.detach().cpu().numpy()

            # pre_prompted_embeddings.append(input_embedding)
            # prompted_embeddings.append(prompted_embedding)
    ...
    # pre_prompted_embeddings = np.array(pre_prompted_embeddings)
    # prompted_embeddings = np.array(prompted_embeddings)

    # pre_prompted_embeddings = pre_prompted_embeddings.reshape(-1, pre_prompted_embeddings.shape[-2],pre_prompted_embeddings.shape[-1])
    # prompted_embeddings = prompted_embeddings.reshape(-1, prompted_embeddings.shape[-2], prompted_embeddings.shape[-1])
